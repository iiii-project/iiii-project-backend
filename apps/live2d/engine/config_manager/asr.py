...
class SherpaOnnxASRConfig(I18nMixin):
    ...
    convert_to_traditional_chinese: bool = Field(
        False, alias="convert_to_traditional_chinese"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "convert_to_traditional_chinese": Description(
            en="Convert recognized Chinese text to Traditional Chinese",
            zh="将识别出的中文文本转换为繁体中文",
        ),
        ...
        "language": Description(
            en="Force recognition to a single language (e.g. 'zh' for SenseVoice); "
            "empty string auto-detects the spoken language",
            zh="强制辨识为单一语言（例如 SenseVoice 用 'zh'）；留空则自动侦测语言",
        ),
    }
