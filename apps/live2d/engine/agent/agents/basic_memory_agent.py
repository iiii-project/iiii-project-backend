...
class BasicMemoryAgent(AgentInterface):
    ...
    def __init__(
        ...
        # 記錄「最後一筆記憶是不是真的用 TTS 講出來的」，interrupt 時只該修正真的被打斷
        # 的發言，silent 的（remember-context 靜默寫入的）不能被 handle_interrupt 蓋掉。
        self._last_entry_silent = False
    ...
    def _add_message(
        ...
        # silent 狀態不放進 message_data 本身——那份 dict 會直接整包送進 LLM API 的
        # messages 參數，多塞一個 API 不認得的欄位沒必要冒風險，另外記在旁邊就好。
        self._last_entry_silent = silent
    ...
    def remember(self, content: str, role: str = "assistant", silent: bool = False) -> None:
        ...
        silent=True marks content that was never actually spoken aloud (e.g. remembered
        in the background while only a short greeting played) — handle_interrupt() must
        not clobber it as if it were a TTS utterance that got cut off."""
        self._add_message(content, role, silent=silent)
    ...
    def handle_interrupt(self, heard_response: str) -> None:
        ...
        if self._memory and self._memory[-1]["role"] == "assistant" and not self._last_entry_silent:
