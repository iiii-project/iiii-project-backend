...
class RealtimeConfig(BaseModel):
    """OpenAI Realtime speech-to-speech settings."""

    enabled: bool = Field(False, alias="enabled")
    model: str = Field("gpt-realtime-2.1", alias="model")
    voice: str = Field("marin", alias="voice")
    input_sample_rate: int = Field(16000, alias="input_sample_rate")
    output_sample_rate: int = Field(24000, alias="output_sample_rate")
...
class CharacterConfig(I18nMixin):
    ...
    realtime_config: RealtimeConfig = Field(
        default_factory=RealtimeConfig, alias="realtime_config"
    )
