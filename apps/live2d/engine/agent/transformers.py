...
from ..utils.traditional_chinese import to_traditional_chinese
...
def response_quality_guard(max_characters: int):
    """Limit response length and remove only clearly duplicated long sentences."""
        ...
        async def wrapper(
            *args, **kwargs
            ...
            async for item in func(*args, **kwargs):
                if not isinstance(item, SentenceWithTags):
                    yield item
                    continue

                text = item.text
                is_long_plain_sentence = (
                    len(text.strip()) >= 20
                    and "\n" not in text
                    and "```" not in text
                    and not item.tags
                )
                if is_long_plain_sentence and text.strip() == previous_text:
                    continue

                remaining = max_characters - emitted_characters
                if remaining <= 0:
                    return
                ...
                previous_text = text.strip() if is_long_plain_sentence else ""
                yield SentenceWithTags(text=text, tags=item.tags)

                if emitted_characters >= max_characters:
                    return
            ...
            *args, **kwargs
            ...
            async for item in stream:
                if (
                    ...
                    display = DisplayText(text=to_traditional_chinese(text))
